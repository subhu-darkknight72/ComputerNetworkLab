...
struct icmphdr *send_recv(int sockfd, char *sendbuf, char *recvbuf, struct iphdr *ip, struct icmphdr *icmp, int *print_flag, struct sockaddr_in *dest, struct sockaddr_in *from, socklen_t fromlen, struct timeval *tv, fd_set *rset, int ttl, double *rtt);
...
void printICMP(struct icmphdr *icmp)
{
    ...
    {
        char *icmpdata = (char *)((char *)icmp + sizeof(struct icmphdr));
        struct iphdr *ip = (struct iphdr *)icmpdata;
        printIP(ip);
        if (ip->protocol == IPPROTO_TCP)
        {
            struct tcphdr *tcp = (struct tcphdr *)(icmpdata + sizeof(struct iphdr));
            printTCP(tcp);
        }
        else if (ip->protocol == IPPROTO_UDP)
        {
            struct udphdr *udp = (struct udphdr *)(icmpdata + sizeof(struct iphdr));
            printUDP(udp);
        }
        else
        {
            fprintf(file, "|                       Unknown Protocol                        |\n");
            fprintf(file, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
        }
    }
}
